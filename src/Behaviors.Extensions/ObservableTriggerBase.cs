...
namespace Behaviors.Extensions
{
    ...
    public abstract class ObservableTriggerBase<TPayload> : TriggerBase, IObserver<TPayload>
    {
        ...
        private IDisposable observableDispose;
        ...
        /// </returns>
        /// <exception cref="BehaviorExtensionException">Method Invoke EventHandler no exists!</exception>
        ...
        {
            ObservableTriggerBase<TPayload> observableTriggerBase =
                (ObservableTriggerBase<TPayload>)obj;
            ...
            } else
            {
                ...
                {
                    observableTriggerBase.UnregisterEvent(
                        sourceNameObject,
                        observableTriggerBase.GetEventName());
                }
        ...
        private void RegisterEvent(object obj, string observableName)
        {
            Debug.Assert(
                this.eventHandlerMethodInfo == null
                && string.Compare(observableName, "Loaded", StringComparison.Ordinal) != 0);
            var targetType = obj.GetType();
            var propertyInfo = targetType.GetProperty(observableName);
            ...
            var methodInfo = typeof(ObservableTriggerBase<TPayload>).GetMethod(
                nameof(this.OnEventImplementation),
                BindingFlags.NonPublic | BindingFlags.Instance);

            Type type = obj.GetType();
            PropertyInfo info = type.GetProperty(observableName);
            if (info == null)
            {
                throw new ArgumentException(
                    string.Format(
            ...
            if (!(info.GetValue(obj, null) is IObservable<TPayload> observable))
            {
                throw new ArgumentNullException(nameof(observable));
            }

           this.observableDispose = observable.Subscribe(this);
